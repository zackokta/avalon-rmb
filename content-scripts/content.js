var content = (function () {
  ...
  function Oe(t, e, n, s = W, r) {
    ...
    return (
      C === null || (C !== at && (C.owned ? C.owned.push(o) : (C.owned = [o]))),
      o
    );
  }
  ...
  function gt(t) {
    for (let e = 0; e < t.observers.length; e += 1) {
      ...
      n.state ||
        ((n.state = me), n.pure ? T.push(n) : B.push(n), n.observers && gt(n));
    }
  }
  ...
  function nn(t) {
    return tn(this, null, function* () {
      const { name: e, mode: n = "closed", css: s, isolateEvents: r = !1 } = t;
      if (!en(e))
        throw Error(
  ...
  async function yt({ target: t, selector: e, detector: n, customMatcher: s }) {
  ...
  function wn(t) {
    ...
    let r;
    for (; (r = s.exec(t)) !== null; ) ((n += r[1]), (e = e.replace(r[1], "")));
    return {
      documentCss: n.trim(),
      shadowCss: e.trim(),
    };
            ...
            const d = () => {
              ...
              typeof p == "function"
                ? (p(
                  ...
                  f && h(),
                  a(!0))
                : f || ((f = !0), (h = s.on("did-change-stream-callbacks", d)));
            };
            d();
          }),
      );
...
`,
  ...
  function ds() {
    return (
      kt ||
      (kt = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])
    );
  }
  ...
  function ys(
    ...
    return (
      ...
      n &&
        i.addEventListener("blocked", (l) => n(l.oldVersion, l.newVersion, l)),
      a
        .then((l) => {
  ...
  class v {
    ...
    async imageCrawlApiCall(e) {
      ...
      return (
        console.log("Pixels from slider origin = " + s.pixelsFromSliderOrigin),
    ...
    async refreshImageCrawl() {
      ...
      for (
        ...
        (await this.getImageSource(v.IMAGE_CRAWL_PUZZLE_IMAGE_SELECTOR)) === e;
      )
  ...
  function Bt(t, e) {
    ...
    return (
      ct(() => {
        (console.log(`Syncing with settings key: "${t}"`),
          R.getSetting(t).then((i) => {
            i !== void 0 && s(() => i);
          }));
  ...
  const _s = "scraperbot",
    xs = {
      ...
      async main(t) {
        ...
        window.addEventListener("VyuSys_Internal_Sync_99", async function (d) {
          ...
          try {
            const u = await Y("webhook", {
              payload: {
                ...
                items: g.items ? JSON.parse(JSON.stringify(g.items)) : void 0,
                detail: g.detail
        ...
        const f = {
          "#__MODERN_ROUTER_DATA__": async (d) => {
            ...
            try {
              ...
              tt(w);
              const b = (await R.getSetting("email")) || "bot@extension.local";
              if (!b) {
                console.error(
        ...
        function h(d) {
          ...
          for (const w in d)
            document.querySelectorAll(w).forEach((b) => {
              (console.log(`Found existing element matching selector: "${w}"`),
                d[w](b));
            });
      ...
      run() {
        e == null &&
          ...
          (e = t.setInterval(() => {
            ...
            s.href !== n.href && (window.dispatchEvent(new nt(s, n)), (n = s));
          }, 1e3)));
      },
    };
  }
  ...
  return (async () => {
    try {
      ...
      throw (
        Is.error('The content script "content" crashed on startup!', t),
        t
      );
    }
  })();
